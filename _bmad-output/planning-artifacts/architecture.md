...
- **Roles:** `admin` flag in Supabase `app_metadata` (server-settable only) + a `profiles` table carrying the account state machine (invited/active/inactive-declined/inactive-withdrawn/deactivated) and contact data.
