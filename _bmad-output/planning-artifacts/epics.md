...
Release allocation: **36 MVP FRs (FR1–FR36), 11 v1.1 FRs (FR37–FR44 and FR46–FR48), and 1 v2 FR (FR45).**
...
- AR11: Single `assets` table with three-value `origin` enum (`ambassador|admin|generated`) from day one; text columns + CHECK constraints instead of pgEnum. Epic 1 provisions two orthogonal organization axes: live-MVP `themes` + composite-unique `asset_themes`, and dormant-v2 `campaigns` + composite-unique `asset_campaigns`. Campaigns include `name`, description, start/end dates, nullable `theme_id REFERENCES themes(id) ON DELETE SET NULL`, and no MVP readers/routes/UI. No `campaign_id` exists on tasks or event tables. Both asset joins are manual-insert-only through explicit admin DAL mutations and are never inferred from upload/task/provenance context. Other v1.1 seams remain provenance/version columns and the `/auth/confirm` purpose parameter.
...
- AR16: FR15 mechanics: `assets.task_id` nullable FK set by upload-init when entering from a task, validated against an open task addressed to the session user; task linkage never writes `asset_themes` or `asset_campaigns`
- AR17: Named indexes for NFR5 (assets created_at DESC, uploader_id, type, partial triaged_at, reverse `asset_themes` and `asset_campaigns` browse indexes, send_records provider_message_id UNIQUE); search = GIN tsvector over asset descriptions only (`simple` config). Theme names are structured filter/browse joins, not full-text search. Theme archive/restore, assignment, and guarded delete lock the theme row; hard delete succeeds at zero connections only, otherwise returns `CONFLICT` with archive as remedy.
...
- AR43: Audit emission via single `audit.emit(tx, …)` in `src/shared` accepting only the closed `AUDIT_EVENTS` registry (dot-notation `entity.verb`, past tense; v1.1 types `asset.shared`/`asset.used_confirmed` in the registry from day one); called inside the mutation transaction; triage verbs, delivery-status updates, and theme CRUD deliberately unaudited; storage-coupled mutations commit DB rows + audit first, storage ops after commit
...
- AR49: Triage intent is route-encoded: `POST/DELETE /api/triage/[assetId]/star` and `/api/triage/[assetId]/themes` set `triaged_at` in the same transaction; `/api/assets/[assetId]/themes` and `/api/assets/bulk-themes` are library curation and never change `triaged_at`. No client-supplied source flag selects behavior.
...
- AR53: Frontend stack: TanStack Query 5 (RSC prefetch + HydrationBoundary), nuqs for URL-canonical filter state, react-hook-form + Zod 4, TanStack Virtual for GalleryGrid; route groups `(ambassador)` (root URLs, `/` → `/tasks`) and `(admin)` (`/admin/*` URLs); optimistic-update verb table: OPTIMISTIC star/unstar and theme assign/unassign; SERVER-ACK dismiss/mark-done/sends/profile edits; NEVER optimistic uploads/processing/exports/consent/deletion/budget
...
- UX-DR8: TriageQueue — full-screen shell on `surface-media` black; large MediaPreview + context line (ambassador · task · date · size) + verb bar + "12 of 40" progress; keyboard verbs: T theme, S star, X dismiss (Z undoes), →/← navigate, space play/pause; all reversible, none confirm; auto-advance after dismiss only; ← revisits with verbs toggleable; processing items allow star/theme assignment, playback waits; queue-clear celebration summary ("40 reviewed, 9 starred, 31 assigned to themes") with Go-to-starred / Back-to-library exits; on-screen buttons mirror every key; keyboard legend until first use; abandoning mid-session restores position ("12 triaged · 28 left")
- UX-DR9: GalleryGrid — square thumbs, scrim'd meta line, star indicator, theme-chip overflow, video duration + play glyph; selection = light-blue ring + black check; hover scrub + quick verbs on desktop, long-press selection on touch; shift-click range, ctrl/cmd toggle; virtualized for thousands of assets with roving tabindex, aria-rowcount, scroll-restore; variants: ambassador My-uploads (delete-own), admin library (full verbs), starred view; empty state with warm illustration + primary action; skeleton loading tiles
...
- UX-DR20: Navigation: ambassador bottom tab bar (Tasks, Upload, My uploads, Profile) with blue active states and light-blue badge counts; admin top bar + left rail (views, theme filter facet, other filter groups), no breadcrumbs; full-screen overlays entered from library, exited Esc/back/X with focus return, never stacked; browser back always works; filter combinations URL-encoded (bookmarkable/shareable)
...
- UX-DR27: Flow 5 offboarding UI mirrors the HR runbook step-for-step (deactivate → filter by ambassador → review → bulk delete → consequence summary → audit evidence); bystander-erasure discovery searches upload descriptions and then asks the uploading ambassador to identify any remaining matches. Curated theme names do not contain personal data and are not a person-search surface.
...
FR15: Epic 5 — Uploads in task context auto-linked to that task without theme/campaign propagation
...
FR24: Epic 6 — Filter by ambassador/type/date/theme, description-only search, and theme browse
FR25: Epic 6 — "New this week" triage queue with preview, active-theme assignment, inline authorized creation, and starring
FR26: Epic 6 — Star/unstar (admin-shared, ambassador-invisible)
FR27: Epic 6 — Theme lifecycle, guarded delete/archive remedy, explicit assignment incl. bulk, and connected-upload browse
...
FR38: Epic 8 — Generated-origin assets support explicit theme assignment/filter/browse and normal library verbs (v1.1)
...
FR45: Epic 10 — Admin-only campaign calendar with nullable selected theme and explicit asset connections (v2)
...
Establishes the EU-pinned application on its confirmed stack (scaffold from the `with-supabase` example, Supabase Stockholm, Vercel arn1, Railway worker shell, structured error-logging seam, CI/CD — Sentry deferred to post-MVP), provisions the complete MVP data foundation including live theme and dormant campaign seams, and delivers the auth spine. After this epic a seeded admin can sign in through one passwordless front door, sessions revoke immediately on demand, Fleet Deck base patterns exist, and every later mutation is role-scoped by the DAL and RLS backstop.
...
Gives admins the curation cockpit: one shared library across all origins with fast composable theme filters and description-only search, the conveyor-belt "new this week" triage queue (keyboard theme/star/dismiss, one motion per item), admin-private starring, curated theme lifecycle and browse, single/bulk theme assignment, and admin brand-asset uploads into the same library. After this epic the library is navigable, searchable, and organized — the surface every export and governance action operates on.
...
**UX:** UX-DR8 (TriageQueue), UX-DR9 (GalleryGrid admin variant), UX-DR12 (ThemePicker), UX-DR20 (theme facet), UX-DR21 (filters/empty/loading)

### Epic 7: Export, Deletion & Governance
Closes the MVP loop and makes the program provably compliant: async multi-GB zip export with the SelectionBar's Theme verb and human-readable filenames; consequence-first permanent deletion; offboarding and bystander erasure with description search plus uploader confirmation; ambassador-initiated withdrawal; backup + tested-restore posture; storage-cost surfacing; and the 6-month audit-expiry job. After this epic "need content → publish-ready export in under 10 minutes" and "calm 15-minute offboarding" both land.
...
### Epic 9: Sharing, Usage Tracking & Program Insights (v1.1)
Turns the library into a provable production line: one-click LinkedIn sharing with caption, post-export publish check-offs, usage notifications and profile counters that close the "did my content matter?" loop, tokenized 1:1 magic links that land ambassadors straight in upload, a top-5 leaderboard, and a stats page plus audit-trail viewer. Builds on Epic 8's usage/provenance data; campaign reporting remains dormant until Epic 10.
**FRs covered:** FR42, FR43, FR44, FR46, FR47, FR48
**NFRs:** —
**Architecture:** AR11 (purpose-scoped token seam), AR12 (durable usage/export events consumed), AR43 (v1.1 audit types emitted), AR46 (export-items check-off seam)
**UX:** v1.1-cycle components — Leaderboard, StatsPage, export check-off prompt, share/caption UI

### Epic 10: Campaign Calendar (v2)
Gives admins an admin-only calendar for creating, viewing, updating, archiving/restoring, and deleting time-boxed campaigns with a name, description, date range, optional selected theme, and explicit many-to-many asset connections. Campaigns can be created before a theme is selected; no task/upload context infers asset connections. The ambassador landing page remains the task list.
**FRs covered:** FR45
**NFRs:** NFR8 (admin-only surface), NFR5 (calendar and connection interactions at expected scale)
**Architecture:** AR11 (dormant `campaigns`/`asset_campaigns` seam, nullable theme FK, manual connections); campaign lifecycle delete-versus-archive implementation semantics finalized in v2
**UX:** Admin-only calendar, campaign editor, optional ThemePicker selection, and explicit asset-connection management; no ambassador-surface changes

---

## Epic 1: Platform Foundation & Passwordless Access

Establishes the EU-pinned application, complete MVP data foundation, and auth spine: one magic-link front door, no passwords, immediate session revocation, a server-only DAL choke point with role separation, the account-state machine, the closed audit emitter, and the RLS backstop — the foundation every later epic is built on.
...
### Story 1.2: Data-layer, account & organization foundation

As the solo developer,
I want the Drizzle migration workflow plus the account, theme, and dormant campaign foundations in place,
So that MVP curation and the v2 calendar build on one governed, reproducible schema.
...
**When** the data-foundation migration is applied,
**Then** `themes` includes stable identity, unique name, timestamps, and nullable `archived_at`, while `asset_themes` contains non-null `asset_id`, `theme_id`, and `created_at`, enforces `UNIQUE(asset_id, theme_id)`, and has its reverse browse index,
**And** `archived_at` is documented as a lifecycle status flag, not soft delete; assignment/lifecycle behavior is implemented in Epic 6.

**Given** campaigns are a dormant v2 axis (FR45, AR11),
**When** the same foundation is applied,
**Then** `campaigns` includes `name`, description, start/end dates, nullable `theme_id REFERENCES themes(id) ON DELETE SET NULL`, lifecycle timestamps, and no requirement for a selected theme at insert; `asset_campaigns` contains non-null `asset_id`, `campaign_id`, and `created_at`, enforces `UNIQUE(asset_id, campaign_id)`, and has its reverse browse index,
**And** there is no `campaign_id` on tasks, usage/event rows, or any table outside `asset_campaigns`; campaign tables have no MVP reader, route, or UI.

**Given** both asset joins require explicit admin intent,
**When** schema and integration tests run,
**Then** duplicate theme/campaign connections are rejected, a campaign can be created with `theme_id = NULL`, and upload/task-linkage fixtures create neither `asset_themes` nor `asset_campaigns` rows.
...
**Then** `audit.emit(tx, …)` lives in runtime-neutral `src/shared`, accepts only types from the closed `AUDIT_EVENTS` registry (dot-notation `entity.verb`, past tense; v1.1 types `asset.shared`/`asset.used_confirmed` present from day one), and is always called inside the mutation transaction with the tx handle.
...
**And** reversible triage verbs (star/theme assign/theme unassign/dismiss), delivery-status updates, and theme CRUD are deliberately unaudited per the FR34 registry.
...
**Given** task linkage and organization connections are orthogonal (FR15, AR11),
**When** task upload-init and commit complete,
**Then** neither path creates, infers, or propagates an `asset_themes` or `asset_campaigns` row, and no campaign identifier is copied to the asset, task, or event data,
**And** integration coverage asserts zero join rows for task-linked uploads even when the task title/context matches an existing theme or campaign.
...
Gives admins the curation cockpit: one shared library with fast composable theme filtering and description search, curated theme lifecycle and browse, the conveyor-belt triage queue, admin-private starring, and brand-asset uploads.
...
**Then** a virtualized GalleryGrid (roving tabindex, aria-rowcount, scroll-restore) shows all assets across origins with square thumbs, scrim'd meta lines, and star/theme indicators, paginated via `{ items, nextCursor }` keyset.
...
**When** I apply ambassador, type, date, and theme filters or search upload descriptions,
...
**And** the admin left rail presents a theme filter facet with no filing hierarchy metaphor.

**Given** the performance target (NFR5, AR17),
**When** filters/search run at thousands of assets,
**Then** results return in < 500 ms, backed by the named indexes and a GIN `tsvector` over asset descriptions only (`simple` config); theme names resolve through the indexed `asset_themes` join and are never included in full-text search.

**Given** theme browse is a structured organization surface (FR24, FR27),
**When** I open an active or archived theme from the filter/browse surface,
**Then** I see all connected uploads, including connections retained while the theme is archived,
**And** curated theme names are not offered or documented as a person-search surface.
...
### Story 6.3: Curated theme lifecycle, assignment & browse

As an admin,
I want to manage curated themes and connect them to assets,
So that the library stays consistently organized without freeform taxonomy drift.

**Acceptance Criteria:**

**Given** the curated theme model (FR27, UX-DR12),
**When** I create, view, rename, archive, or restore a theme,
**Then** the operation uses the stable `themes` row from Story 1.2, theme CRUD is deliberately outside the closed audit registry, and archive/restore locks the row in the mutation transaction.

**Given** I assign themes outside triage,
**When** I use the ThemePicker on one asset or the SelectionBar on a multi-select batch,
**Then** `/api/assets/[assetId]/themes` or `/api/assets/bulk-themes` explicitly inserts/removes `asset_themes` rows, the UI patches theme assign/unassign optimistically and reconciles on invalidate, and neither route changes `triaged_at`.

**Given** the ThemePicker assignment control (FR25, FR27, UX-DR12),
**When** it opens,
**Then** it type-aheads over active curated themes, supports multi-assign and removable selections, and authorized admins get an inline “Create theme” affordance that returns the new active theme without leaving the current flow.

**Given** a theme is archived,
**When** assignment or browse surfaces load,
**Then** it is absent from ThemePicker choices and every new assignment rejects it, while its existing connections remain visible in filters, theme browse, and connected-upload views until restored,
**And** automated coverage proves archived-theme picker exclusion and server-side assignment rejection rather than relying on UI hiding alone.

**Given** a theme hard-delete request (FR27, AR17),
**When** the guarded DAL mutation locks the theme row and checks connections in one transaction,
**Then** deletion succeeds only when `asset_themes` has zero connections; a connected theme returns `CONFLICT` with archive as the remedy and preserves the row and joins,
**And** concurrency/tests cover both the zero-connection success and connected-theme conflict-remedy paths so archive-versus-assign and check-versus-delete races cannot violate the rule.
...
**When** I press T (theme) / S (star) / X (dismiss) / →/← (navigate) / space (play-pause),
**Then** every verb is reversible in place and confirmation-free; T opens the active-only ThemePicker with authorized inline creation; dismiss (`markTriaged()`, never sharing a code path with deletion) auto-advances with Z-undo; and ← revisits triaged items with verbs still toggleable.

**Given** triage intent is encoded by the route namespace (AR49),
**When** theme assignment runs through `POST/DELETE /api/triage/[assetId]/themes`,
**Then** the join mutation and `triaged_at` update commit in the same transaction, while `/api/assets/[assetId]/themes` and `/api/assets/bulk-themes` never touch `triaged_at`,
**And** no client-supplied source flag or shared mutation side effect selects triage behavior.
...
**Then** theme assignment/star still work while playback waits on the designed placeholder, and clearing the queue shows a celebration summary ("40 reviewed, 9 starred, 31 assigned to themes") with Go-to-starred / Back-to-library exits; abandoning mid-session restores position ("12 triaged · 28 left").
...
**Then** it reuses the Epic 4 upload/commit/transcode pipeline with `origin: admin` set from the session role, appears in the shared library, and supports explicit theme assignment, theme filtering/browse, starring, and export like any asset with zero schema change,
**And** upload/commit never infers a theme or campaign connection from the admin upload context.
...
**Then** the SelectionBar reads “{n} selected · {~size} · Theme · Export zip · Delete · Clear” and shows a size estimate first ("40 files · ~4.3 GB"), `POST /api/exports` creates `export_records` + `export_items` and enqueues `export_jobs`, the worker streams originals into a zip in the `exports` bucket, progress polls at 5 s, and I can navigate away and get an "export ready" notice.
...
**Then** the flow follows the runbook step-for-step — deactivate (sends stop + sessions revoked, from Epic 2) → filter the library by that ambassador → review → select-all → bulk delete → consequence summary → confirm.

**Given** an erasure request concerns a bystander without an account (UX-DR27 and the adopted runbook Process B),
**When** I discover affected uploads,
**Then** I search upload descriptions and ask the uploading ambassador(s) to identify any remaining matches before deleting the confirmed assets and generated children,
**And** curated theme names are documented as containing no personal data and are never used as a person-search surface; description search plus uploader confirmation is the accepted discovery procedure.
...
**Then** it appears in the shared library with `origin: 'generated'` (the enum value provisioned in MVP) and supports explicit theme assignment, theme filtering/browse, starring, and export exactly like any asset,
**And** generation sources, prompts, and provenance edges create neither `asset_themes` nor `asset_campaigns`; every organization connection requires a separate explicit admin action.
...
Turns the library into a provable production line: sharing, usage notifications, tokenized links, leaderboard, and the non-campaign stakeholder receipt. Builds on Epic 8's usage/provenance data; MVP already records durable usage/export events and the closed v1.1 audit registry. Campaign calendar UI and campaign reporting remain in Epic 10.
...
**Then** I receive a usage notification rendered as a CelebrationMoment with specific publication context (for example, "your photo — used in the recruitment post") and my profile usage counter ticks up,
**And** an `asset.used_confirmed` event is emitted.

### Story 9.4: Tokenized 1:1 deep-link tasks
...
### Story 9.6: Stats page & audit-trail viewer
...
**When** I open it,
**Then** it shows uploads over time, percentage used, and top content — computable from the durable MVP KPI columns and usage/export events — as the stakeholder receipt,
**And** it contains no campaign reporting until the v2 calendar and explicit campaign connections ship in Epic 10.
...
Gives admins a dedicated calendar for time-boxed campaign planning, optional theme selection, explicit asset connections, and campaign reporting. It activates the dormant schema from Story 1.2 without changing the ambassador task-list home or inferring organization from tasks/uploads.

> **v2 note:** These stories define the v2 capability against the dormant MVP schema seam. Calendar interaction detail is finalized in the v2 design cycle; the data and intent boundaries below are fixed.

### Story 10.1: Admin-only campaign calendar & lifecycle

As an admin,
I want to create and manage time-boxed campaigns in a calendar,
So that program work has an explicit planning surface separate from ambassador tasks.

**Acceptance Criteria:**

**Given** the dormant campaign foundation from Story 1.2 (FR45, AR11),
**When** I create a campaign,
**Then** I provide a required `name`, description, start date, and end date, may leave `theme_id` null, and the campaign appears on an admin-only calendar spanning its date range,
**And** automated coverage proves a campaign can be created before a theme is chosen and rejects an end date before its start date.

**Given** campaign lifecycle management (FR45),
**When** I view, update, archive, restore, or delete a campaign,
**Then** archive removes it from active calendar defaults without destroying its row/connections, restore returns it, and hard delete removes its `asset_campaigns` joins plus the campaign in one transaction without deleting any assets,
**And** Epic 10's migration replaces the dormant schema's protective campaign-delete restriction with the chosen transaction-safe lifecycle behavior.

**Given** role separation (NFR8),
**When** an ambassador session requests campaign/calendar routes or data,
**Then** the DAL returns `FORBIDDEN`, no campaign navigation appears, and the ambassador landing page remains `/tasks`.

### Story 10.2: Optional campaign theme & explicit asset connections

As an admin,
I want to select at most one theme and explicitly connect assets to a campaign,
So that campaign organization is intentional and remains independent of request workflow context.

**Acceptance Criteria:**

**Given** optional theme selection (FR45),
**When** I create or edit a campaign,
**Then** I may select zero or one theme from the curated theme list, clearing the selection writes `NULL`, and hard-deleting the selected theme sets `campaigns.theme_id` to `NULL` without deleting the campaign.

**Given** campaign asset management (FR45),
**When** I add or remove assets from the campaign editor,
**Then** explicit admin mutations insert/remove composite-unique `asset_campaigns` rows and connected assets are visible from the campaign,
**And** task creation, task-linked upload, spontaneous/admin upload, and generated-asset provenance never create or infer campaign joins; integration coverage asserts no joins from `assets.task_id` or other workflow context.

### Story 10.3: Campaign reporting activation

As an admin,
I want reporting scoped to explicit campaign connections,
So that campaign results are trustworthy rather than inferred from loosely related tasks or events.

**Acceptance Criteria:**

**Given** the base StatsPage from Story 9.6 and explicit campaign connections (FR45; FR48 v2 activation clause),
**When** campaign reporting is enabled,
**Then** the stats page can filter and summarize by campaign using only `campaigns` and explicit `asset_campaigns` membership plus durable asset usage/export events,
**And** no `campaign_id` is added to tasks or usage/event tables and no report infers membership from `assets.task_id`.
