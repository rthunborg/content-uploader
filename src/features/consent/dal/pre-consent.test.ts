...
vi.mock("server-only", () => ({}));
const mocks = vi.hoisted(() => ({ requirePre: vi.fn(), accept: vi.fn(), read: vi.fn(), parse: vi.fn(), hash: vi.fn() }));
vi.mock("@/lib/auth", () => ({ requireUserPreConsent: mocks.requirePre }));
vi.mock("./acceptance", () => ({ acceptCurrentTermsAndActivate: mocks.accept }));
...
beforeEach(() => { mocks.requirePre.mockReset().mockResolvedValue({ userId: "00000000-0000-4000-8000-000000000001", accountState: "invited" }); mocks.accept.mockReset().mockResolvedValue({ id: "record" }); mocks.read.mockReset().mockResolvedValue({ id: "terms", payload: { cards: [] }, payloadSha256: "trusted" }); mocks.parse.mockReset().mockReturnValue({ success: true, data: { cards: [1, 2, 3] } }); mocks.hash.mockReset().mockReturnValue("trusted"); });
