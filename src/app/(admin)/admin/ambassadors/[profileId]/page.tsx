...
export default async function AmbassadorPage({ params }: { params: Promise<{ profileId: string }> }) { const { profileId } = await params; const rail = <nav aria-label={ambassadorCopy.navigationLabel}><Link className="inline-flex min-h-11 items-center p-3 text-[#3344dd] underline focus-visible:outline-2" href="/admin/ambassadors">{ambassadorCopy.rosterLink}</Link></nav>; try { return <AdminShell rail={rail}><AmbassadorDetail profile={await getProfileForAdmin(profileId)} /></AdminShell>; } catch (error) { if (error instanceof DomainError && error.code === "NOT_FOUND") notFound(); throw error; } }
