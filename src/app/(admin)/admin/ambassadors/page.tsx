...
export function RosterPageContent({ page, cursor }: { page: AmbassadorPage; cursor?: string }) {
  ...
  return <><h1 className="mb-6 text-2xl font-semibold">{ambassadorCopy.heading}</h1><AmbassadorRoster page={page} /></>;
}
export default async function AmbassadorsPage({ searchParams }: { searchParams: Promise<{ cursor?: string }> }) {
  const { cursor } = await searchParams;
  try { return <AdminShell rail={rail}><RosterPageContent page={await listAmbassadors(cursor)} cursor={cursor} /></AdminShell>; }
